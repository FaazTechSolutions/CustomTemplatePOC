let _this = this;
const response = _this.ResponseData; // Add JSON response here
let detailsChartInstance = null;

function renderTable(response) {
  const table = document.getElementById("monthlyDetailsTable");
  if (!response || !response.Data.Sheets || response.Data.Sheets.length === 0)
    return;
  ...
  let thead = `
        <thead>
            <tr class="title-row">
                <th colspan="${3 + weekCols.length}">Monthly Details</th>
            </tr>
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (row.kpiSpan > 0) {
      let kpiText = row.KPIs ? String(row.KPIs).replace(/\n/g, "<br>") : "";
  ...
  initChart(data, weekCols, "monthlyChart");
}

function initChart(data, weekCols, canvasId) {
  const selector = document.getElementById("monthlyDetailsSelector");
...
function updateChart(rows, weekCols, ctx, groupKey, canvasId) {
  ...
  const datasets = rows.map((row, i) => {
    ...
    const isDatasetPercentage = subj.includes("%") || subj.includes("persentage") || subj.includes("percentage") || subj.includes("rate") || subj.includes("sla") || subj.includes("نسبة");
