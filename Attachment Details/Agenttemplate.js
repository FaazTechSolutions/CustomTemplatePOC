...
const CONFIG = {
  ...
  UPLOAD_BASE: 'https://portal.mawarid.com.sa/apps4x-api/api/v1/attachment/{{Local.CompanyId}}/upload',
...
let $modalTitle, $modalSubtitle, $modalIconWrap, $modalFilesLoading, $modalFilesEmpty;
...
async function loadAttachments() {
  ...
  // $btnRefresh.addClass('loading').prop('disabled', true);
...
function createTile(att, index) {
  ...
  const isReq = att.Mandatory ?? false;
...
async function uploadFile() {
  ...
  try {
    ...
    await $.ajax({
      url: buildUploadUrl(att, selectedFile),
      method: 'POST',
...
function attachSVG(size = 14) { return `<svg width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>`; }
...
(function boot() {
  ...
  function init() {
    ...
    $btnRefresh[0].addEventListener('click', () => setTimeout(loadAttachments, 0));
    $btnRetry[0].addEventListener('click', () => setTimeout(loadAttachments, 0));

    $modalClose[0].addEventListener('click', closeModal);
    $btnCancel[0].addEventListener('click', closeModal);

    /* Stop bubbling to Angular host */
    $modalBackdrop[0].addEventListener('click', function (e) { e.stopPropagation(); });
    document.getElementById('modal-card').addEventListener('click', function (e) { e.stopPropagation(); });
    ...
    /* ── Drop zone: drag-and-drop (click-to-browse is native via input overlay) ── */
    const dropEl = $dropZone[0];
    if (dropEl) {
      dropEl.addEventListener('dragover', function (e) { e.preventDefault(); dropEl.classList.add('drag-over'); }, { passive: false });
      dropEl.addEventListener('dragenter', function () { dropEl.classList.add('drag-over'); });
      dropEl.addEventListener('dragleave', function () { dropEl.classList.remove('drag-over'); });
      dropEl.addEventListener('dragend', function () { dropEl.classList.remove('drag-over'); });
  ...
  if (typeof Zone !== 'undefined' && Zone.root && typeof Zone.root.run === 'function') {
    Zone.root.run(init);
  } else {
    init();
  }
