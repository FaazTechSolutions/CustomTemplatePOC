...
(function ($) {
  ...
  var STATUS_ROWS = [
    {
      ...
      label: "Backed Out",
      key: "Count_BackedOut",
      opts: { valueColor: "var(--rsr-red)" },
    },
    { code: "C", label: "Not Started", key: "Count_SelectedNotStarted" },
    { code: "D", label: "UNDER MEDICAL", key: "Count_UnderMedical" },
    {
      code: "E",
      label: "MEDICAL DONE WAITING FITNESS",
      calc: function (d) {
        return (
          (d.Count_UnderMedicalTreatment || 0) +
          (d.Count_PrometricExam || 0) +
          (d.Count_Mumaris || 0)
        );
      },
    },
    { code: "F", label: "MEDICAL FIT", key: "Count_MedicalFit" },
    {
      ...
      opts: { valueColor: "var(--rsr-red)", labelColor: "var(--rsr-red)" },
    },
    {
      code: "H",
      label: "UNDER MEDICAL TREATMENT",
      key: "Count_UnderMedicalTreatment",
    },
    {
      code: "J",
      label: "SVP / QVP",
      calc: function (d) {
        return (d.Count_SVP || 0) + (d.Count_QVP || 0);
      },
    },
    ...
    { code: "N", label: "VISA STAMPED", key: "Count_Stamped" },
    {
      ...
      calc: function (d) {
        return (d.Count_TicketConfirmed || 0) + (d.Count_TicketBooking || 0);
      },
    },
  ];
  ...
  function rsrRenderTable() {
    ...
    filteredItems.forEach(function (item, idx) {
      var headerCode = formatHeaderCode(
        item.SubProjectCodes || "SubProj_" + (idx + 1),
      );
      var hpCode = item.HiringProject || item.Customer || "";
      html +=
        '<th style="text-align:center;min-width:170px">' +
        headerCode +
        "<br>" +
    ...
    var mpCodes = {};
    filteredItems.forEach(function (item, idx) {
      mpCodes[idx] = item.MainProjectCode || "-";
    });
    mpCodes.total = "-";
    addRow("Main Project Code", mpCodes, {
      bold: true,
      valueColor: "var(--rsr-teal)",
    });
    ...
    html +=
      '<tr style="background:var(--rsr-gold-bg);border-top:1px solid var(--rsr-gold-lt)"><td style="font-weight:600;color:var(--rsr-gold)">Remaining (This Month) &mdash; calculated: Target &minus; Arrived</td>' +
      '<td style="text-align:center;font-weight:700;color:var(--rsr-gold)" id="rsrSingleRemainTotal">0</td>';
    filteredItems.forEach(function (item, idx) {
      html +=
        '<td style="text-align:center;font-weight:600;color:var(--rsr-gold)" id="rsrRemain_single_' +
        idx +
        '">0</td>';
    });
    html += "</tr>";
    ...
    html +=
      '<tr style="background:var(--rsr-gold-bg)"><td style="font-weight:600;color:var(--rsr-gold)">Completion (This Month) % &mdash; calculated: (Arrived / Target) &times; 100</td>' +
      '<td style="text-align:center;font-weight:700;color:var(--rsr-gold)" id="rsrSingleCompTotal">0.0%</td>';
    filteredItems.forEach(function (item, idx) {
      html +=
        '<td style="text-align:center;font-weight:600;color:var(--rsr-gold)" id="rsrComp_single_' +
        idx +
        '">0.0%</td>';
    });
    html += "</tr>";
    ...
    html +=
      '<tr style="border-bottom:1.5px solid var(--rsr-gold-lt)"><td style="color:var(--rsr-gold);font-weight:700">Target (this month) &mdash; [Editable Input]</td>' +
    ...
    html +=
      ...
      '" class="rsr-rpt-hdr">Status &mdash; fixed order, always shown, no show/hide toggle</td></tr>';
    ...
    addRow("Confirmed Completion (This Month)", confirmedVals, {
      bg: "var(--rsr-green-bg)",
      labelColor: "var(--rsr-green)",
      valueColor: "var(--rsr-green)",
      bold: true,
    });

    // 17. Confirmed Completion (This Month) % = (Confirmed / Target) * 100
    html +=
      '<tr><td style="font-weight:600;color:var(--rsr-green)">Confirmed Completion (This Month) % &mdash; calculated: (Confirmed / Target) &times; 100</td>' +
      '<td style="text-align:center;font-weight:700;color:var(--rsr-green)" id="rsrSingleConfCompTotal">0.0%</td>';
    filteredItems.forEach(function (item, idx) {
      html +=
        '<td style="text-align:center;font-weight:600;color:var(--rsr-green)" id="rsrConfComp_single_' +
        idx +
        '">0.0%</td>';
    });
    html += "</tr>";
  ...
  function rsrExportExcel() {
    ...
    var style = `<style>
      ...
      th { background-color: #F5F3EE; color: #888780; border: 1px solid #E3E3E3; padding: 10px; text-align: left; font-weight: bold; text-transform: uppercase; }
      td { border: 1px solid #E3E3E3; color: #4A4845; padding: 8px; vertical-align: middle; }
      .rsr-rpt-hdr { background-color: #F5F3EE !important; color: #888780 !important; font-weight: bold; text-transform: uppercase; }
